...
namespace atom_rviz
{
ImageDisplayWithClickBase::ImageDisplayWithClickBase() : Display(), sub_(), tf_filter_(), messages_received_(0)
{
  topic_property_ =
...
ImageDisplayWithClickBase::~ImageDisplayWithClickBase()
{
  ImageDisplayWithClickBase::unsubscribe();
}

void ImageDisplayWithClickBase::onInitialize()
{
  it_.reset(new image_transport::ImageTransport(update_nh_));
  scanForTransportSubscriberPlugins();
}

void ImageDisplayWithClickBase::setTopic(const QString& topic, const QString& datatype)
{
...
void ImageDisplayWithClickBase::incomingMessage(const sensor_msgs::Image::ConstPtr& msg)
{
...
void ImageDisplayWithClickBase::failedMessage(const sensor_msgs::Image::ConstPtr& msg,
...
void ImageDisplayWithClickBase::reset()
{
  Display::reset();
  if (tf_filter_)
  {
...
void ImageDisplayWithClickBase::updateQueueSize()
{
...
void ImageDisplayWithClickBase::subscribe()
{
  if (!isEnabled())
  {
    return;
    ...
    if (!topic_property_->getTopicStd().empty() && !transport_property_->getStdString().empty())
    {
      ...
      if (targetFrame_.empty())
      {
        sub_->registerCallback(
            boost::bind(&ImageDisplayWithClickBase::incomingMessage, this, boost::placeholders::_1));
      }
      else
      {
        ...
        tf_filter_->registerFailureCallback(boost::bind(
            &ImageDisplayWithClickBase::failedMessage, this, boost::placeholders::_1, boost::placeholders::_2));
      }
    }
...
void ImageDisplayWithClickBase::unsubscribe()
{
  tf_filter_.reset();
  sub_.reset();
}

void ImageDisplayWithClickBase::fixedFrameChanged()
{
  if (tf_filter_)
  {
...
void ImageDisplayWithClickBase::scanForTransportSubscriberPlugins()
{
...
void ImageDisplayWithClickBase::updateTopic()
{
  unsubscribe();
  reset();
  subscribe();
...
void ImageDisplayWithClickBase::fillTransportOptionList(EnumProperty* property)
{
