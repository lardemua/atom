...
The **eye_in_world_system** is a robotic system used to evaluate hand eye calibrations in ATOM.
...
First launch the gazebo simulation:
    ...
    roslaunch eye_in_world_system_gazebo gazebo.launch

Then you can bringup the system:

    roslaunch eye_in_world_system_bringup bringup.launch

You can record a bag file using:

    roslaunch eye_in_world_system_bringup record.launch
...
In this section, out goal is to describe the calibration pipeline for the **eye_in_world_system**.
...
Download the bagfile and put it in **$ROS_BAGS/eye_in_world_system**.
...
Using ATOM conventions, we define name of the calibration package as **eye_in_world_system_calibration**, and create it using:

    rosrun atom_calibration create_calibration_pkg --name eye_in_world_system_calibration

**NOTE**: This procedure is carried out only once, and was done already. As such, this ros package is already [included in the atom repo](https://github.com/lardemua/atom/tree/noetic-devel/atom_examples/eye_in_world_system/eye_in_world_system_calibration). Therefore, you **should not execute this instruction** for the eye_in_world_system.


## Configuring the calibration

This is the [config.yml](https://github.com/lardemua/atom/blob/noetic-devel/atom_examples/eye_in_world_system/eye_in_world_system_calibration/calibration/config.yml) that we wrote to define the calibration. There is a single sensor to be calibrated, named **rgb_hand**. The pattern is a charuco marker.
The configuration file points to the train bagfile mentioned above, and the _anchored_sensor_ is not defined.

To configure run:

    rosrun eye_in_world_system_calibration configure

Which will run a series of checks and produce several files inside the **eye_in_world_system_calibration** package.

The configuration produces a [visual schematic summarizing](https://github.com/lardemua/atom/blob/noetic-devel/atom_examples/eye_in_world_system/eye_in_world_system_calibration/calibration/summary.pdf) the calibration you have setup.
...
To collect a dataset we run:
    ...
    roslaunch eye_in_world_system_calibration collect_data.launch output_folder:=$ATOM_DATASETS/eye_in_world_system/dataset1 overwrite:=true
...
Download and decompress the dataset to **$ATOM_DATASETS/eye_in_world_system/train**.
...
To calibrate, first setup visualization with:
    ...
    roslaunch eye_in_world_system_calibration calibrate.launch
...
Then carry out the actual calibration including noise, using:
    ...
    rosrun atom_calibration calibrate -json $ATOM_DATASETS/eye_in_world_system/train/dataset.json -v -rv -nig 0.1 0.1
...
Download the [test](https://drive.google.com/file/d/16Vi6xo6kt2wEeFBWbTGrZdOL8DmT5ztt/view?usp=sharing) and decompress to **$ATOM_DATASETS/eye_in_world_system/test**.

    roslaunch eye_in_world_system_calibration full_evaluation.launch test_json:=$ATOM_DATASETS/eye_in_world_system/test/dataset.json train_json:=$ATOM_DATASETS/eye_in_world_system/train/atom_calibration.json
