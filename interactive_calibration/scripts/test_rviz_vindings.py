...
if __name__ == '__main__':
    ...
    rviz_file = interactive_calibration_path + '/templates/config.rviz'

    rviz = yaml.load(open(rviz_file), Loader=yaml.FullLoader)
    ...
    new_vm_displays = deepcopy(vm_displays)
    new_vm_displays = []

    for i, vm_display in enumerate(vm_displays):
        print('Display ' + str(i) + ' is ' + vm_display['Name'])
        if vm_display['Class'] == 'rviz/RobotModel':
            print(vm_display)
            new_vm_displays.insert(i, vm_display)

    vm['Displays'] = new_vm_displays

    yaml.dump(rviz, open(interactive_calibration_path + '/templates/config_new.rviz', 'w'))

    # lines = []
    # fd = open(interactive_calibration_path + '/templates/config.rviz', 'r+')
    ...
    #
    #
    # display_index = lines.index('Visualization Manager:')
...
#         config = rviz.Config()
#         reader.readFile(config, interactive_calibration_path + '/templates/config.rviz')
...
#         display0 = self.manager.getRootDisplayGroup().getDisplayAt(0)
#         print(display0)
#
#
#         exit(0)
