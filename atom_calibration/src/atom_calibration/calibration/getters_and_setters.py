...
def getterTransform(dataset, transform_key, collection_name):
    ...
    trans = dataset["collections"][collection_name]["transforms"][transform_key][
        "trans"
    ]
...
def setterTransform(dataset, values, transform_key, collection_name=None):
    ...
    ):  # if collection_name is None, set all collections with the same value
        for collection_key in dataset["collections"]:
            dataset["collections"][collection_key]["transforms"][transform_key][
                "trans"
            ] = trans  # set the translation
            dataset["collections"][collection_key]["transforms"][transform_key][
                "quat"
            ] = quat  # set the quaternion
    else:
...
def setterJointParam(dataset, value, joint_key, param_key, collection_name):
    if (
        ...
        for collection_key in dataset["collections"]:
            dataset["collections"][collection_key]["joints"][joint_key][param_key] = (
                value[0]
            )
    else:
