...
def errorReport(dataset, residuals, normalizer, args):
    ...
    for collection_key in keys:
        ...
        for sensor_key, sensor in dataset["sensors"].items():
            keys = [
                ...
                if ("c" + collection_key) == k.split("_")[0] and sensor_key in k
            ]
            v = [
                residuals[k] * normalizer[sensor["modality"]]
                for k in keys
                if residuals[k]
            ]
            if v:
    ...
    for col_idx, _ in enumerate(table_header):
        if col_idx == 0:
            bottom_row.append(
                Fore.BLUE + Style.BRIGHT + "Averages" + Fore.BLACK + Style.RESET_ALL
            )
    ...
    for col_idx, _ in enumerate(table_header):
        ...
        table.rows[max_row_idx][col_idx] = (
            Fore.RED + table.rows[max_row_idx][col_idx] + Style.RESET_ALL
        )
    ...
    if args["save_calibration_errors"]:
        with open(
            args["output_folder"] + "/calibration_errors.csv", "w", newline=""
        ) as f_output:
...
@Cache(args_to_ignore=["_dataset"])
def getDepthPointsInPatternAsNPArray(
...
@Cache(args_to_ignore=["_dataset"])
def getPointsInSensorAsNPArray(
    ...
    idxs = _dataset["collections"][_collection_key]["labels"][_pattern_key][
        _sensor_key
    ][_label_key]
...
def getPointsInDepthSensorAsNPArray(
    ...
    img = getDepthImageFromDictionary(
        _dataset["collections"][_collection_key]["data"][_sensor_key]
    )
    idxs = _dataset["collections"][_collection_key]["labels"][_pattern_key][
        _sensor_key
        ...
        for pattern_key, pattern in dataset["calibration_config"][
            ...
            for sensor_key, sensor in dataset["sensors"].items():  # iterate all sensors
                ...
                if sensor["modality"] == "rgb":
                    ...
                    w, h = (
                        collection["data"][sensor_key]["width"],
                        collection["data"][sensor_key]["height"],
                    )
                    K = np.ndarray(
                    ...
                    for idx, label_idx in enumerate(
                        ...
                        r[rname] = (
                            np.sqrt(
                                (pts_in_image[0, idx] - pts_detected_in_image[0, idx])
                                ** 2
                                + (pts_in_image[1, idx] - pts_detected_in_image[1, idx])
                                ** 2
                            )
                            / normalizer["rgb"]
                        )
                    ...
                    for idx in range(0, pts_in_image.shape[1]):
                        idxs_projected.append(
                            {"x": pts_in_image[0][idx], "y": pts_in_image[1][idx]}
                        )
                    # store projections
                    ...
                    if (
                        "idxs_initial"
                        not in collection["labels"][pattern_key][sensor_key]
                    ):
                        collection["labels"][pattern_key][sensor_key][
                            "idxs_initial"
                ...
                # elif sensor['msg_type'] == 'LaserScan':
                    ...
                    rhos = [
                        collection["data"][sensor_key]["ranges"][idx] for idx in idxs
                    ]
                    thetas = [
                    ...
                    from_frame = dataset["calibration_config"]["calibration_pattern"][
                        "link"
                    ]
                    to_frame = sensor["parent"]
                    pattern_to_sensor = getTransform(
                        from_frame, to_frame, collection["transforms"]
                    )
                    ...
                    pts.extend(
                        patterns[pattern_key]["frame"]["lines_sampled"]["bottom"]
                    )
                    ...
                    extrema_right = np.reshape(
                        pts_in_chessboard[0:2, 0], (2, 1)
                    )  # longitudinal -> ignore z values
                    rname = (
                        collection_key
                        + "_"
                        + "p_"
                    ...
                    edges2d_in_chessboard = pts_in_chessboard[
                        0:2, collection["labels"][sensor_key]["edge_idxs"]
                    ]  # this
                    ...
                    # Transform the pts from the pattern's reference frame to the sensor's reference frame -----------------
                    from_frame = sensor["parent"]
                    to_frame = dataset["calibration_config"]["calibration_pattern"][
                        "link"
                    ]
                    ...
                    if args["ros_visualization"]:
                        marker = [
                            x
                            for x in dataset_graphics["ros"][
                                "MarkersLaserBeams"
                            ].markers
                            if x.ns == str(collection_key) + "-" + str(sensor_key)
                        ][0]
                        marker.points = []
                        rviz_p0_in_laser = Point(
                            p0_in_laser[0], p0_in_laser[1], p0_in_laser[2]
                        )
                ...
                # elif sensor['msg_type'] == 'PointCloud2':
                    ...
                    for idx in collection["labels"][pattern_key][sensor_key]["samples"]:
                        ...
                        r[rname] = (
                            float(abs(points_in_pattern[2, idx]))
                            / normalizer["lidar3d"]
                        )
                    ...
                    for idx in collection["labels"][pattern_key][sensor_key]["samples"]:
                        ...
                        rname = rname_pre + str(idx)
                        r[rname] = (
                            float(abs(points_in_pattern[2, idx])) / normalizer["depth"]
                        )
