...
def createDepthMarkers3D(dataset, sensor_key, collection_key, pattern_key, color=(255, 255, 0, 0.5),
    ...
    if cached:
        ...
        points = getPointsInDepthSensorAsNPArrayNonCached(
            collection_key, pattern_key, sensor_key, 'idxs', dataset)
    ...
    marker = Marker(
        header=Header(frame_id=frame_id, stamp=stamp),
        ns=namespace + '-limit_points', id=0, frame_locked=True, type=Marker.CUBE_LIST,
        action=Marker.ADD, lifetime=rospy.Duration(0),
        pose=Pose(
...
def setupVisualization(dataset, args, selected_collection_key):
    ...
    rospy.init_node("odometry_inspector")
    ...
    # ----------------------------------------------------------------------------------------
    # Create 3D Labels  (only for reference sensor lidar3d or depth)
    # Note: Republish new labeled point clouds because the labels must be redrawn as they change position
    # ----------------------------------------------------------------------------------------
    graphics['PubPointCloudReference'] = dict()
    for sensor_key, sensor in dataset['sensors'].items():
        # print('Sensor ' + sensor_key)

        if not sensor_key == args['reference_sensor']:
            continue
        ...
        for collection_key, collection in dataset['collections'].items():
            ...
            if sensor['modality'] == 'lidar3d':
                ...
                final_pointcloud_msg = PointCloud2(
                    ...
                    row_step=original_pointcloud_msg.row_step, data=original_pointcloud_msg.data,
                    is_dense=original_pointcloud_msg.is_dense)

                topic = generateLabeledTopic(dataset['sensors'][sensor_key]['topic'], type='3d')
                topic += '_reference'
                graphics['sensors'][sensor_key]['PubPointCloudReference'] = rospy.Publisher(
                    topic, PointCloud2, queue_size=0, latch=True)
                graphics['sensors'][sensor_key]['collections'][collection_key][
                    'PointCloudsReference'] = final_pointcloud_msg
        ...
        for collection_key, collection in dataset['collections'].items():
            ...
            if sensor['modality'] == 'lidar2d':
                ...
                marker = Marker(
                    ...
                    color=ColorRGBA(
                        r=graphics['colormap']['range'][collection_key]['color'][0],
                        g=graphics['colormap']['range'][collection_key]['color'][1],
                        b=graphics['colormap']['range'][collection_key]['color'][2],
                        a=1.0))
            ...
            # if sensor['msg_type'] == 'PointCloud2':  # -------- Publish the velodyne data ------------------------------
                ...
                labeled_topic = generateLabeledTopic(
                    dataset['sensors'][sensor_key]['topic'], type='3d')
...
def visualizationFunction(models, selection):
    ...
    reference_collection_key = args['reference_collection']
...
#
    ...
    for pattern_key in dataset['calibration_config']['calibration_patterns'].keys():
        for sensor_key in graphics['labels'][pattern_key]:
            ...
            markers = createDepthMarkers3D(
                ...
                cached=False, frame_id=None, namespace=sensor_key)

            graphics['sensors'][sensor_key]['PubDepthMarkers'].publish(markers)

    # Update timestamp and colors for reference pointcloud2 message
    for sensor_key in graphics['sensors']:
        ...
        pointcloud_msg = graphics['sensors'][sensor_key]['collections'][reference_collection_key]['PointCloudsReference']
        ...
        for idx, point in enumerate(list(pc2.read_points(pointcloud_msg))):
            ...
            # colorize reference point cloud in blue
            r, g, b = 0, 0, 200
            rgb = struct.unpack('I', struct.pack('BBBB', b, g, r, 255))[0]

            sensor_idx = list(dataset['collections'][reference_collection_key]
                              ['labels'][pattern_key].keys()).index(sensor_key)
        ...
        graphics['sensors'][sensor_key]['PubPointCloudReference'].publish(pointcloud_msg_labeled)

    # Update timestamp and colors for pointcloud2 message
    for sensor_key in graphics['sensors']:
        if not dataset['sensors'][sensor_key]['modality'] in ['lidar3d']:  # TODO add  lidar2d
            continue
        ...
        points = []  # Build a list of points.
        for pattern_key in dataset['calibration_config']['calibration_patterns'].keys():
            idxs = dataset['collections'][selected_collection_key]['labels'][pattern_key][
                sensor_key]['idxs']
            idxs_limit_points = dataset['collections'][selected_collection_key]['labels'][
                pattern_key][sensor_key]['idxs_limit_points']
            sensor_idx = list(dataset['collections'][selected_collection_key]
                              ['labels'][pattern_key].keys()).index(sensor_key)
