...
if __name__ == "__main__":
    ...
    # robot = URDF.from_xml_file(rospack.get_path('atom_calibration') + "/urdf/atlas_macro.urdf.xacro")
    ...
    cmap = cm.Pastel1(np.linspace(0, 1, len(xml_robot.sensors)*2))
    # cmap = cm.tab10(np.linspace(0, 1, len(xml_robot.sensors)))
    ...
    for i, xml_sensor in enumerate(xml_robot.sensors):
        ...
        for parent, child in zip(chain[0::], chain[1::]):
            ...
            if parent == xml_sensor.calibration_parent and child == xml_sensor.calibration_child:
                ...
                g2.edge(parent, child, color=rgb, style='dashed', _attributes={'penwidth': '1'})

    # get the type of this joint: fixed, revolute, prismatic, etc
    # for i, joint in enumerate(xml_robot.joints):
    #     if joint.child == 'model':
    #         continue
    #
    #     if not (joint.parent == 'ground'):
    #         continue
    #
    #     if joint.type == 'fixed':
